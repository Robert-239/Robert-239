...
def predict_seq(model,window, input_sequence : np.ndarray, predict_length):
    ...
    for _ in range(predict_length):
        ...
        sequence = sequence[:,-window:].reshape((1,window))
    ...
    sequence = sequence.reshape((1,18))
...
def generate_ML_SMA_cross_over_signals(candle_df : pl.DataFrame ,window ,model_s,model_l):
    ...
    scaler.fit(short_avg)
    print(short_avg) 
    short_avg = scaler.transform(short_avg)
    print(short_avg)
    ...
    long_avg = scaler.transform(long_avg)
    ...
    for i in range(6):
        ...
        print(f"short out = {short_out} - long out = {long_out}\n") 
    ...
    short_seq = scaler.inverse_transform(short_seq[:,-window:])
    print("short seq")
    print(short_seq)
    
    long_seq = scaler.inverse_transform(long_seq[:,-window:])
    print("long seq")
    print(long_seq)
...
def simulate_stream(price_data, short_ma=30, long_ma=60, stop_loss=None):
    ...
    portfolio_dict2 = portfolio2.row(0, named=True)
    portfolio_dict2['portfolio_value'] = portfolio_dict2['capital'] + portfolio_dict2['asset_value']
    ...
    for i in tqdm(range(len_df), desc="\nProcessing Historical Data Stream \n", leave=True):
        if i != (len_df - 1):
            ax1.clear()
            ax2.clear()
        ...
        # Use generate_ML_SMA_cross_over_signals
        ...
        buy_amount2 = get_buy_amount(pred_avg_signal)
        portfolio_dict2, trade2 = trade_proc(candle_dict, portfolio2, pred_avg_signal['positions'], amount=buy_amount2, stop_loss=stop_loss)

        # Log the trade
        log_trade(trade2)

        # Adjust trade dictionary keys to match TRADE_SCHEMA
        trade2_adjusted = {
            'date': trade2['date'],
            'price': trade2['price'],
            'buy': trade2['buy'],
        ...
        # Record trades
        current_trade = pl.DataFrame([trade2_adjusted], schema=TRADE_SCHEMA)
        trades.extend(current_trade)

        # Update portfolio
        temp_frame = pl.DataFrame([portfolio_dict2], schema=PORTFOLIO_SCHEMA)
        portfolio2.extend(temp_frame)
    ...
    ax2.plot(np.asarray(portfolio2['date'], dtype='datetime64[s]'), portfolio2['portfolio_value'], label="Portfolio Value", color=c)
    ...
    portfolio_values = portfolio2['portfolio_value'].to_numpy()
    performance['num_trades'] = return_sig.filter((pl.col('positions') == 1) | (pl.col('positions') == -1)).shape[0]
    performance['performance'] = portfolio2['portfolio_value'].item(-1) / portfolio2['portfolio_value'].item(1)
    ...
    prediction_file = f"predicted_values_from_model.csv"
    ...
    return portfolio2, trades, ml_signals, performance
...
if __name__ == "__main__":
    ...
    price_data_D = load_and_combine_csv(FILE_LIST,'2023_dayly_price.csv')
    print(price_data_D)
    price_data_8H = load_and_combine_csv(FILE_LIST,'2023_8_hour_price.csv')
    print(price_data_8H)
    price_data_4H = load_and_combine_csv(FILE_LIST,'2023_4_hour_price.csv')
    print(price_data_4H)
